...
test.describe("主要機能E2E", () => {
  ...
  test("日報作成と一覧検索・カテゴリ絞り込み", async ({ page }) => {
    ...
    await expect(page.getByText(`検索: ${reportTitle}`)).toBeVisible();
  ...
  test("日報編集", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/reports\/[^/]+$/);
    await expect(page.getByRole("heading", { name: updatedReportTitle })).toBeVisible();
  });

  test("チーム作成と参加コード入力", async ({ page }) => {
    await page.goto("/team");
    await expect(page.getByRole("heading", { level: 1, name: "チーム" })).toBeVisible();
    ...
    await expect(page.getByText("チームを作成しました！🎉")).not.toBeVisible();
  ...
  test("日報削除", async ({ page }) => {
    ...
    await page.getByRole("searchbox", { name: "検索" }).fill(updatedReportTitle);
    await page.getByRole("button", { name: "検索" }).click();
    await expect(page.getByText("該当する日報がありません。")).toBeVisible();
  });
});
